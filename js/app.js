var model = function() {
    ...
    for (var i = 0; i <=150000; i++) {
        var randomInt1 = Math.floor(Math.random() * (area.length - 1)); 
        var randomInt2 = Math.floor(Math.random() * (area[randomInt1].cities.length - 1)); 
    ...
    for (var i = 0; i <=100000; i++) {
        ...
        var randomInt1 = Math.floor(Math.random() * (bankList.length - 1)); 
...
function KoViewModel() {
   ...
   self.actualCash.subscribe(function(value){
    ...
    self.actualNote.extend({ rateLimit: 50 });
